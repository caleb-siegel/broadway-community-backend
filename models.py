...
class Show_Preference(db.Model, SerializerMixin):
    ...
    seat_region = db.Column(db.String)
    start_date = db.Column(db.Date, default=date.today)
    end_date = db.Column(db.Date, default=date.today)
    show_time = db.Column(db.Time, default=time(0, 0))
...
class Show_Info (db.Model, SerializerMixin):
    ...
    lottery_url = db.Column(db.String)
...
class Ticket_Info (db.Model, SerializerMixin):
    ...
    price = db.Column(db.Numeric(scale=2))
    seat_region = db.Column(db.String)
    seat_numbers = db.Column(db.String)
    number_of_seats = db.Column(db.Integer)
    show_time = db.Column(db.Time, default=time(0, 0))
    show_date = db.Column(db.Date, default=date.today)
